{@Suppress(TypeNameRule("*"), All, "Legacy Code")}
unit V6.UI.TestComplete.OpenApplication.Classes;
{——————————————————————————————————————————————————————————————————————————————
                  DELPHI 32 source code
                  Copyright 1995 Soft Tech (NZ) Ltd
———————————————————————————————————————————————————————————————————————————————
———————————————————————————————————————————————————————————————————————————————

            UNIT: tcopenAppClasses
     DESCRIPTION: Unit required to allow V6 to be compiled as an 'Open Application'
                  for TestComplete 5. For QA to use V6 as an 'Open Application', they
                  need to copy 'tcOpenHook.dll' to the application folder as well as
                  run V6 with the '/OpenForTestComplete' command-line parameter.
         PROJECT: V6
 ORIGINAL AUTHOR: Automated QA, slightly modified by Shaune Tappan
         CREATED: 9/2007
...
begin
  ...
  if VMT <> nil then
  begin
    VMT := Pointer(NativeInt(VMT) + vmtParent + SizeOf(Pointer));
...
var
  Entry: PtcTableEntry;
begin
  Entry := FItems^[(NativeInt(Key) and $7FFFFFFF) mod FCapacity];
  while Entry <> nil do
  begin
...
begin
  Index := (NativeInt(Key) and $7FFFFFFF) mod FCapacity;
  Entry := FItems^[Index];
  PrevEntry := nil;
  while Entry <> nil do
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Index := (NativeInt(Entry.Key) and $7FFFFFFF) mod FCapacity;
...
begin
  ...
  if AParamInfo <> nil then
    Result := PParamInfo(NativeInt(AParamInfo) + SizeOf(AParamInfo^) - c_ShortStringLength + Length(AParamInfo.Name));
end;
...
begin
  Result := Pointer(NativeInt(Pointer(AClass)) - SizeOf(TVmt));
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      HeaderEnd := Pointer(NativeInt(MethodHeader) + MethodHeader.Len);
      Params := PParamInfo(NativeInt(MethodHeader) + SizeOf(MethodHeader^) - c_ShortStringLength + Length(MethodHeader.Name) + SizeOf(TReturnInfo));
      Param := Params;
      if Integer(Param) < NativeInt(HeaderEnd) then
      begin
        ParamList := TList.Create;
        while (NativeInt(Param) < NativeInt(HeaderEnd)) and not IsReturnExtraParam(Param) do
        begin
          ParamList.Add(Param);
...
begin
  ...
  if MethodHeader <> nil then
    Result := PReturnInfo(NativeInt(MethodHeader) + SizeOf(MethodHeader^) - c_ShortStringLength + Length(MethodHeader.Name));
end;
